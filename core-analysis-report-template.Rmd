...
if (processed_meta$filtering == "miQC filtered" ){
  ...
  # Plot filtering
  filtered_cell_plot <- miQC::plotFiltering(pre_processed_sce, processed_meta$miQC_model)
  # Combine plots
  filtered_cell_plot <-
...
cluster_col <- grep("louvain|walktrap|kmeans", colnames(colData(processed_sce)))
cluster_name <- colnames(colData(processed_sce)[cluster_col])
